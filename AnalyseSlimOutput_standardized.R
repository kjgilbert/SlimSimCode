...
## where names(mut.id.dat) <- c("mut.ID", "mut.type", "base_position", "seln_coeff", "dom_coeff", "subpop_ID", "generation_arose", "mut.prev")
...
## where names(poly.mut.dat) <- c("mut.ID", "mut.type", "base_position", "seln_coeff", "dom_coeff", "subpop_ID", "generation_arose", "mut.prev")
...
setwd("~/Documents/My_Documents/UofToronto/SLiM/Outputs/Selfing_N10000outputs")

sample.output.files <- c(
	"SampleOutput_Sep16_N10000_del_self99_rep1.txt",
#	"SampleOutput_Sep16_N10000_del_self99_rep2.txt",
#	"SampleOutput_Sep16_N10000_del_self99_rep3.txt",
#	"SampleOutput_Sep16_N10000_del_self99_rep4.txt",
	"SampleOutput_Sep16_N10000_del_self99_rep5.txt",
	"SampleOutput_Sep16_N10000_del_self90_rep1.txt",
#	"SampleOutput_Sep16_N10000_del_self90_rep2.txt",
#	"SampleOutput_Sep16_N10000_del_self90_rep3.txt",
#	"SampleOutput_Sep16_N10000_del_self90_rep4.txt",
	"SampleOutput_Sep16_N10000_del_self90_rep5.txt"
	)
	
full.output.files <- c(
	"FullOutput_Sep16_N10000_del_self99_rep1.txt",
#	"FullOutput_Sep16_N10000_del_self99_rep2.txt",
#	"FullOutput_Sep16_N10000_del_self99_rep3.txt",
#	"FullOutput_Sep16_N10000_del_self99_rep4.txt",
	"FullOutput_Sep16_N10000_del_self99_rep5.txt",
	"FullOutput_Sep16_N10000_del_self90_rep1.txt",
#	"FullOutput_Sep16_N10000_del_self90_rep2.txt",
#	"FullOutput_Sep16_N10000_del_self90_rep3.txt",
#	"FullOutput_Sep16_N10000_del_self90_rep4.txt",
	"FullOutput_Sep16_N10000_del_self90_rep5.txt"
	)
...
for(i in 1:length(sample.output.files)){
	...
	fixed.mut.id.start <- head(tail(unlist(strsplit(system(paste(c("grep -n Mutations ", sample.file), collapse=""), intern=TRUE), split=":")), n=2), n=1)
	generations <- as.numeric(matrix(unlist(strsplit(system(paste(c("grep -n ' R ' ", sample.file), collapse=""), intern=TRUE), split=" ")), ncol=5, byrow=TRUE)[,2])

### quick fix for now because fixations are in there... as last, 3rd to last, and 5th to last
	mut.id.starts <- as.numeric(unlist(strsplit(system(paste(c("grep -n Mutations ", sample.file), collapse=""), intern=TRUE), split=":"))[seq(1, (2*((num.gens.sampled+3)-1)), by=2)])
	poly.mut.id.starts <- mut.id.starts[-c(8,10,12)]
	...
	## fixed data output
	fixeddat <- read.table(sample.file, skip=fixed.mut.id.start)
	names(fixeddat) <- c("mut.ID", "mut.type", "base_position", "seln_coeff", "dom_coeff", "subpop_ID", "gen_arose", "gen.fixed")
	...
	for(j in 1:num.gens.sampled){
		...
		if(j < num.gens.sampled){
			...
			genodat <- read.table(sample.file, skip=genomes.starts[j], nrow=num.inds.sampled, sep="A")
			...
			results[iterate ,] <- c(sample.file, gen, theta, pi.stats, pi.stats[2]/pi.stats[3], mut.stats, rep(NA, 4))
			iterate <- iterate + 1
		}
