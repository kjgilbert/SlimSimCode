...
for(i in gens){
	...
	if(i == tail(gens, n=1)){	# then it's the last gen (10N)
		...
		full.samp.inds.start <- as.numeric(strsplit(system(paste(c("grep -n Individuals ", taking.in.file), collapse=""), intern=TRUE), split=":")[[1]][1])
		full.samp.genomes.start <- as.numeric(strsplit(system(paste(c("grep -n Genomes ", taking.in.file), collapse=""), intern=TRUE), split=":")[[1]][1])
		full.samp.file.end <- as.numeric(head(tail(unlist(strsplit(system(paste(c("wc -l ", taking.in.file), collapse=""), intern=TRUE), split=" ")), n=2), n=1))

		# sample 100 inds out of the 10,000 pop in final file (20,000 chromosomes)
		odd.nums <- seq(1, (pop.size * 2), by=2)
		sub.samp1 <- sort(sample(odd.nums, size=num.inds.sampled, replace=FALSE))
		sub.samp2 <- sub.samp1+1
		file.lines.to.sample1 <- full.samp.genomes.start + sub.samp1
		file.lines.to.sample2 <- full.samp.genomes.start + sub.samp2
		
		# take only the desired lines, put them into the 'modified' file (those files are being made for every gen, to get an SFS then average over the sfs)
...
for(j in gens){
	...
	# subsamp 10 inds from genodat
	odd.nums <- seq(1, (100 * 2), by=2)
	sub.samp1 <- sort(sample(odd.nums, size=10, replace=FALSE))
	sub.samp2 <- sub.samp1+1
	sub.samp <- sort(c(sub.samp1, sub.samp2))
	gdat <- gdat[sub.samp ,]
