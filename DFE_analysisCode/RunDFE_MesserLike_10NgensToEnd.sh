...
do
	...
	Rscript CommandLine_Convert_SlimToDFE-alpha_avgOverTime_10NtoEnd.R $base_name $dir $genosize $popsize
done
...
do
	...
	mkdir $dir1
	# make a config file
	Rscript MakeConfigFiles_DFE_2epochs_AvgSFS_10NtoEnd.R $do_beneficial $dir $input -0.1 0.5
	# run DFE
		# run class 0
