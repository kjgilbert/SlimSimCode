...
setwd("/cap1/kgilbert/DFE_alpha/Outputs")

#setwd("/cap1/kgilbert/DFE_alpha/Oct11_2epoch_DFE_Results_rescaleUandS_N1000")
...
results <- data.frame(matrix(nrow=length(class0folders), ncol=21))
names(results) <- c("group.ID", "generation", "pop.size", "genome.size", "mut.types", "mating.system", "replicate", "N1.0", "N2.0", "t2.0", "Nw.0", "f0.0", "L.0", "N1.1", "N2.1", "t2.1", "Nw.1", "b", "Es", "f0.1", "L.1")
...
for(i in 1:length(class0folders)){
	...
	spec.IDs <- c(unlist(strsplit(rep.ID, split="_"))[c(2,4:8)])
...
write.table(results, file="est_dfe_results_2epoch_Nov10-transMateSys_throughTime.csv", sep=",", col.names=TRUE)
