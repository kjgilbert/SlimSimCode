...
# Dec 16 transition mating systems with 25 mbp genome

dir <- "/cap1/kgilbert/RunningR_SlimResults/Dec16_MateSysTransition"
setwd(paste(c(dir, "/SlimOutputs"), collapse=""))

sample.files <- system("ls SampleOutput_Dec16_N10000_25mbp_*", intern=TRUE)
fixed.files <- system("ls FixedOutput_Dec16_N10000_25mbp_*", intern=TRUE)
...
gens.to.sample.at <- format(c((pop.size*10)+1, seq((pop.size*10.01),(pop.size*10.5), by=150),
	seq((pop.size*10.5), (pop.size*13), by=5000)), scientific=FALSE)
...
output.file <- "RelativeLoadCalculated_Dec16_transitionMateSys.csv"
