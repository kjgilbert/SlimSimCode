source('/cap1/kgilbert/RunningR_SlimResults/Jan4_N1000_SlimOutputsFiles/SourceFilesR/MeanFitness_deletOnlyAndFull_MeanNumMuts_sRange_after4Ngens.R', chdir = TRUE)


# Nov19


setwd("/cap1/kgilbert/RunningR_SlimResults/Jan4_N1000_SlimOutputsFiles")

sample.files <- system("ls *escale*/SampleOutput_Jan4*_N1000_25mbp_*", intern=TRUE)
fixed.files <- system("ls *escale*/FixedOutput_Jan4*_N1000_25mbp_*", intern=TRUE)
full.files <- system("ls *escale*/FullOutput_Jan4*_N1000_25mbp_*", intern=TRUE)
...
# these have more pairs so don't have to do any more calculations later :)
effect.size.min <- c(-1, -1, -1, -0.1, -0.01, -0.001, -0.0001, -0.00001, -0.0001)		# the MOST negative in the range wanted
effect.size.max <- c(0,	-0.1, -0.01, -0.01, -0.001, -0.0001, -0.00001, 0, 0)		
...
output.file <- "MutationLoad_Jan4-N1000.csv"
