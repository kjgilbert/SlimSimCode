...
# for HOMOZYGOTES
	...
	names(hom.window.num.bendel.muts) <- c("individual", paste("s.window.hom.bendel.muts", min.s, max.s, sep="_"))
		...
		# also count number of neutral homozygous muts
	hom.window.num.neut.muts <- data.frame(matrix(NA, ncol=(1+length(min.s))))
	names(hom.window.num.neut.muts) <- c("individual", paste("s.window.hom.neut.muts", min.s, max.s, sep="_"))
...
# for HOMOZYGOTES -- INCLUDE ALL FIXATIONS
	...
	names(homAndFixed.window.num.bendel.muts) <- c("individual", paste("s.window.homAndFixed.bendel.muts", min.s, max.s, sep="_"))
		...
		# also count number of neutral homozygous muts
	homAndFixed.window.num.neut.muts <- data.frame(matrix(NA, ncol=(1+length(min.s))))
	names(homAndFixed.window.num.neut.muts) <- c("individual", paste("s.window.homAndFixed.neut.muts", min.s, max.s, sep="_"))
	...
	total.neut.num.hom.muts <- 0
	per.ind.neut.num.hom.muts <- NULL
	...
	for(k in seq(1, (pop.size*2), by=2)){			# go through individuals within generation within simulation replicate
		...
		num.hom.neut.muts <- length(poly.mut.dat$seln_coeff[intersect(which(poly.mut.dat$mut.ID %in% hom.muts), which(poly.mut.dat$seln_coeff == 0))])
...
#	(2 categories of homs because can include or not include fixations in the pop from fixed data)
		...
		# also get numbers of homozygous neutral mutations:
		hom.window.num.neut.muts[iterate.inds ,] <- c(iterate.inds, rep(num.hom.neut.muts, length(min.s)))
	...
	if(is.null(fixed.mut.dat)){			# but if nothing fixes:
		...
		homAndFixed.window.num.neut.muts <- hom.window.num.neut.muts		
	}else{
		...
		for(ll in 1:length(min.s)){
			...
			num.fixed.neut.muts.in.bin <- length(fixed.mut.dat$seln_coeff[fixed.mut.dat$seln_coeff == 0])
			...
			homAndFixed.window.num.neut.muts[, ll+1] <- homAndFixed.window.num.neut.muts[, ll+1] + num.fixed.neut.muts.in.bin
	...
	mean.hom.num.neut.muts <- apply(hom.window.num.neut.muts[, - which(names(hom.window.num.neut.muts) == "individual")], FUN=mean, MARGIN=2)
	...
	mean.homAndFixed.num.neut.muts <- apply(homAndFixed.window.num.neut.muts[, - which(names(homAndFixed.window.num.neut.muts) == "individual")], FUN=mean, MARGIN=2)
	...
	return(c(
		...
		mean.homAndFixed.num.neut.muts
	))
}
