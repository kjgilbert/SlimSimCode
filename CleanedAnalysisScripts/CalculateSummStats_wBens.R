...
source('/cap1/kgilbert/NewSims_MesserLikeParams/CalculateTheta.R', chdir = TRUE)
...
source('/cap1/kgilbert/NewSims_MesserLikeParams/CalculateDeletMutsPerInd_withBens.R', chdir = TRUE)
...
source('/cap1/kgilbert/NewSims_MesserLikeParams/CalculateMeanFitness.R', chdir = TRUE)
	...
	for(i in 1:length(sample.output.files)){
		...
		for(j in 1:num.gens.sampled){
			...
			if(j < num.gens.sampled){
				...
				temp.results <- c(sample.file, format(as.numeric(gen), scientific=FALSE), theta, pi.stats, pi.stats[2]/pi.stats[3], mut.stats, fitness.stats)
